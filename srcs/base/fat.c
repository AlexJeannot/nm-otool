...
void getSubFile32(t_env *env, void *file, uint32_t arch_pos)
{
    ...
    if (!(controlOverflow(env->file.size, subfile_offset)))
        errorExit("Overflow on 32-bits FAT binary subfile header offset", env->target.name[env->target.id]);
    ...
    if (!(controlOverflow(env->file.size, subfile_offset + env->file.subsize)))
        errorExit("Overflow on 32-bits FAT binary subfile size", env->target.name[env->target.id]);
...
void getSubFile64(t_env *env, void *file, uint32_t arch_pos)
{
    ...
    uint64_t            arch_header_offset;
    uint64_t            subfile_offset;
    ...
    if (!(controlOverflow(env->file.size, subfile_offset)))
        errorExit("Overflow on 64-bits FAT binary subfile header offset", env->target.name[env->target.id]);

    env->file.subsize = ifSwapuInt64(env->fathdr.s_bytes, arch_header->size);
    if (!(controlOverflow(env->file.size, subfile_offset + env->file.subsize)))
        errorExit("Overflow on 64-bits FAT binary subfile size", env->target.name[env->target.id]);
